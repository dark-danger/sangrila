...
function RegistrationFormContent() {
    ...
    const AVAILABLE_EVENTS = ["Group Dance", "Fashion Show"];

    // State for the selected event to handle programmed selection
    const [selectedEvent, setSelectedEvent] = useState(() => {
        if (eventParam) {
            const event = events.find(e => (e.id === eventParam || e.name === eventParam) && AVAILABLE_EVENTS.includes(e.name));
    ...
    const handleProceedToPayment = () => {
        ...
        if (totalMembers < 5) {
            setStatus("error");
            setMessage("Only entry allowed if you have 5 or more members. Please add more members.");
                    ...
                    <div className="text-center mb-8 md:mb-12">
                        ...
                        <div className="space-y-4 text-muted-foreground font-medium max-w-3xl mx-auto text-xs sm:text-sm leading-relaxed">
                            ...
                            <motion.div
                                ...
                                className="inline-flex items-center gap-2 px-4 py-2 mt-2 rounded-full bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-bold"
                            >
                                <AlertCircle className="w-4 h-4" />
                                Only entry allowed if you have 5 or more members
                            </motion.div>
                                        ...
                                        <div className="space-y-2 group">
                                            ...
                                            <div className="relative">
                                                ...
                                                >
                                                    ...
                                                    {events.filter(e => AVAILABLE_EVENTS.includes(e.name)).map(e => <option key={e.id} value={e.name} style={{ background: '#1a1b2e', color: '#ffffff' }}>{e.name} ({e.fee})</option>)}
                                                </select>
                                            </div>
                                        </div>
                                    </div>
